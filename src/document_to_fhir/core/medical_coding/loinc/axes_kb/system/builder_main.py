...
"""Main script to build the LOINC Axis System Knowledge Base.

This script reads LOINC data, uses a Gemini model to process it, and generates
a CSV file containing the System KB.
"""
...
from src.document_to_fhir.core.medical_coding.loinc.axes_kb.system import builder
...
_MAX_RANK = flags.DEFINE_integer("max_rank", 2000, "Maximum LOINC rank.")
_LIMIT = flags.DEFINE_integer(
    "limit", 0, "Limit number of rows (0 for none)."
)
_WORKERS = flags.DEFINE_integer("workers", 10, "Number of parallel workers.")
...
def main(argv):
  ...
  if _LIMIT.value > 0:
    filename = (
        f"loinc_axis_system_kb_top_{_MAX_RANK.value}_limit_{_LIMIT.value}.csv"
    )
  else:
    filename = f"loinc_axis_system_kb_top_{_MAX_RANK.value}.csv"
  ...
  if df_loinc is None or df_loinc.empty:
    logging.exception("Failed to load any records. Check path and criteria.")

  logging.info("Starting System KB build...")
  try:
  ...
  builder_instance = builder.SystemKBBuilder(
      df_loinc=df_loinc,
      client=llm_client,
