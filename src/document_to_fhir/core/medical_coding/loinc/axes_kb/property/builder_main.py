...
"""Main script to build the LOINC Axis Property Knowledge Base.

This script reads LOINC data, uses a Gemini model to extract properties,
...
from src.document_to_fhir.core.medical_coding.loinc.axes_kb.property import builder
...
def main(argv):
  ...
  if _LIMIT.value > 0:
    filename = (
        f"loinc_axis_property_kb_top_{_MAX_RANK.value}_limit_{_LIMIT.value}.csv"
    )
  else:
    filename = f"loinc_axis_property_kb_top_{_MAX_RANK.value}.csv"
  ...
  logging.info("Initializing PropertyKBBuilder...")
  try:
  ...
  builder_instance = builder.PropertyKBBuilder(
      df_loinc=df_loinc,
      client=llm_client,
